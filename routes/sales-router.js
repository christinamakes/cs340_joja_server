...
router.put('/update-s', function(req,res,next){
    ...
    const queryUpdateSale = `UPDATE Sales SET ? WHERE order_number = ?`;
          ...
          // Run the 1st query
    db.connection.query(queryUpdateSale, [{member_id:member_id,employee_id:(employee_id === 0 ? 'NULL' : employee_id),purchase_date:purchase_date,invoice_total:invoice_total},order_number], function(error, rows, fields){
        if (error) {
